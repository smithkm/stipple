...
public class RecursiveTileSet<T> extends TileSet<T> {
	...
	public static <T> RecursiveTileSet<T> generate(int colours, int n, int k, Random rand) {
		...
		newSequences: while (true) {
			 ...
			 * System.err.println("Generating subtiles"); if(!t.generateSubTiles(rand))
			 * continue newSequences; // Start over with new mappings } break; // We got
			 * recursive tilings for all of the tiles
			 */
		}
