...
public class Node {
	...
	public Node(Set<Node> adjacent, Node downstream, Coordinate location, int rainfall) {
		super();
	...
	public Optional<Node> flowToOneOf(final Set<Node> connected) {
		...
		ToDoubleFunction<Optional<Double>> weight = theta -> {
			return Math.pow((Math.cos(theta.orElse(0.0)) + 1) / 2, 100)
					- Math.pow((Math.cos(theta.orElse(0.0)) + 1) / 2, 300) / 4;
		};
	...
	public void cutAdjacency(Geometry g) {
		adjacent.removeIf(node -> {
			LineString segment = g.getFactory()
					.createLineString(new Coordinate[] { getLocation(), node.getLocation() });
