...
public class Stippler<Point> implements Supplier<Point>, Serializable {
	 ...
	 * @param maxTriesPerRadius Number of consecutive failed attempts before
	 *                          reducing radius
	 * @param decay             Size of the next radius relative to the current.
	 *                          Must be less than 1 and greater than 0.
	 */
	public Stippler(DistanceMetric<Point> metric, Supplier<Point> generator, double radius, int maxTriesPerRadius,
			double decay) {
	 ...
	 */
	public Stippler(DistanceMetric<Point> metric, Supplier<Point> generator, double radius, int maxTriesPerRadius,
			double decay, int maxPoints) {
		this(metric, generator, radius, maxTriesPerRadius, decay, new ArrayList<>(maxPoints));
	}

	private Stippler(DistanceMetric<Point> metric, Supplier<Point> generator, double radius, int maxTriesPerRadius,
			double decay, List<Point> points) {
		super();
	...
	/**
	 * Converts the generated points into a JTS MultiPoint with the rank of each
	 * Point having its rank as its userData field.
	 * 
	 * @param convert Function converting a Stippler Point into a JTS Point
	 * @return
	 */
